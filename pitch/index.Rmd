...
NIOdata <- NIOsentiment %>% select(date, title, URL) %>% arrange(desc(date))

NIOdata <- NIOdata%>% mutate(date=dmy(date) 
                            )
...
library(DT)
datatable(NIOdata, escape = FALSE, filter = list(position = "top"), 
          options = list(
            columnDefs = list(
